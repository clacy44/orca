...
A new worktree requires `--name`; `--from` must name your Run-bound coordinator pane (omitting it auto-binds a pane that may be busy). Remote `current` and `new-child` stay invalid because those words are ambiguous across runtimes; supply an exact discovered remote worktree selector or `new-top-level` with an explicit remote repo selector. The worker's `orchestration check` receives coordinator mail across the federation link as normal inbox delivery, not prompt injection, so `worker_done`, `question`, and `escalation` flow home unchanged — but only while the Dispatch is active: after `worker_done`, `send --to dispatch:<id>` returns `dispatch_inactive`. Give a new worker ~60s before judging it stuck: for the first ~20s `worker-read` shows the preamble and task sitting in the agent's input box with no assistant turn, which is the TUI still starting — the runtime submits the prompt itself, so do not inject a keystroke to "help" (a premature `terminal send` races that submit). The one case that genuinely needs a keystroke is a peer whose agent has not trusted the worktree path: it stalls on the folder-trust prompt before the agent's first turn while `worker-show` still reads `ready`. A raw `terminal send` does not work here over a federation-peer grant — answer it instead with `orca orchestration worker-answer-prompt --dispatch <dispatch_id> --choice accept_trust --environment <peer>`, which works for a `full` grant too and does not race the host's own liveness check the way a raw keystroke does. If the peer is re-paired mid-Dispatch its identity key rotates and in-flight federated workers become unreachable until you refresh the saved environment with `orca environment add`.

**S10-19 least-privilege peer profile — what is different over a federation-peer grant, not a full one.** `worker-start --terminal <handle>` is refused at a peer (omit it — a peer may never name a pre-existing pane, only create a fresh top-level worktree or dispatch into a repo the peer host's `federationDispatchRepos` explicitly allowlists). `terminal send` is refused at a peer entirely: use `worker-answer-prompt` for a startup prompt and `orchestration send --to dispatch:<id>` for anything else you need the worker to see. A peer-owned pane closes the instant its agent exits, so there is no "hand the same pane a follow-up task" flow — start a new Dispatch instead. Any command not on the peer allowlist returns a typed `forbidden` naming the local, non-peer alternative rather than a generic error.

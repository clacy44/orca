...
describe('useTabGroupWorkspaceModel terminal activation focus', () => {
  ...
  it('revokes local terminal state only after the paired-host close confirms (B7)', async () => {
    ...
    // Why: B7 (closeTerminalTab) deliberately reverses the old optimistic
    // contract this test used to assert — pruning local state BEFORE the
    // host confirmed is exactly what let a refused/failed close resurrect
    // the tab on the next snapshot. Arm the host close to succeed so this
    // test can assert the new order: revoke only after confirmation.
    mocks.closeWebRuntimeSessionTab.mockResolvedValue(true)
    const { useTabGroupWorkspaceModel } = await import('./useTabGroupWorkspaceModel')
    const model = useTabGroupWorkspaceModel({ groupId: 'group-1', worktreeId: 'wt-1' })

    model.commands.closeOthers(firstUnified.id)

    await vi.waitFor(() => expect(mocks.closeTab).toHaveBeenCalled())
    ...
    // Why call-order, not just "both were called": the whole point of B7 is
    // that revocation happens strictly after the host's confirmation lands.
    expect(mocks.closeWebRuntimeSessionTab.mock.invocationCallOrder[0]).toBeLessThan(
      mocks.closeTab.mock.invocationCallOrder[0]
    )
  })

  it('does not revoke local terminal state when the paired-host close is refused', async () => {
    const secondTerminal = {
      id: 'terminal-2',
      ptyId: 'remote:env-1@@pty-2',
      worktreeId: 'wt-1',
    ...
    mocks.isWebRuntimeSessionActive.mockReturnValue(true)
    mocks.closeWebRuntimeSessionTab.mockResolvedValue(false)
    const { useTabGroupWorkspaceModel } = await import('./useTabGroupWorkspaceModel')
    const model = useTabGroupWorkspaceModel({ groupId: 'group-1', worktreeId: 'wt-1' })

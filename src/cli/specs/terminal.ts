...
export const TERMINAL_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    summary: 'Report whether a terminal agent is working, gated, or idle',
    usage: 'orca terminal agent-status --terminal <handle> [--environment <peer>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    notes: [
      'status is permission when the agent is waiting at an interactive gate, working while it takes a turn, idle at its ready prompt, and unknown when the runtime has no verdict.',
      'Passive: it reads the terminal and never writes to it.',
      'Use --environment to ask a connected Orca server about one of its own terminals.'
    ],
    examples: [
      'orca terminal agent-status --terminal term_abc123 --json',
      'orca terminal agent-status --terminal term_abc123 --environment laptop'
    ]
  },
  {
    path: ['terminal', 'wait'],

...
describe('chairs-succession handlers', () => {
  ...
  describe('chairs succession-accept', () => {
    ...
    // [G1-10z R2-L6] takeoverCommittedDespiteThrow (N2's guidance line) had no test at all.
    it.each([
      'outstandingDeliveryReadFailed',
      'runGenerationReadFailed',
      'confirmedAuditFailed',
      'takeoverCommittedDespiteThrow'
    ])('prints guidance for the %s warning, not just the bare identifier', async (warning) => {
      const call = vi.fn().mockResolvedValue({
        result: {
          successionId: 'succ_xyz',
          chair: 'chair-a',

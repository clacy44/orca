...
import { CLAUDE_IDLE } from '../../../shared/agent-title-core'
 ...
 *  detector cannot be the authority here. Only Claude's own status glyphs are — a bare spinner
 *  glyph is NOT sufficient: any TUI/wrapper animates those glyphs, so they prove activity, not
 *  identity (agent-title-status.ts:228-237). */
export function isLaunchedClaudePromptTitle(rawTitle: string): boolean {
  const t = rawTitle.trim()
  return t === CLAUDE_IDLE || t.startsWith(`${CLAUDE_IDLE} `)
}

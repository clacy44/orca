...
=== CLI COMMANDS ===
  ...
  # pushing, deleting, or rewriting shared state. Orca may also type a one-line
  # notice into this pane when mail arrives, but do not rely on it: on some hosts
  # that write never lands, so mail you never check is a correction you never
  # see. This cadence stops when you send worker_done; the section below governs
  # from then on.

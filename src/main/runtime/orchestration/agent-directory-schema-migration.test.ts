...
describe('SCHEMA v32 -> v33 migration', () => {
  ...
  it('M1: migration is idempotent — opening twice is a no-op the second time', () => {
    ...
    // Why 39, not 33: SCHEMA_VERSION has moved past this suite's original v33 several times
    // since this assertion was written (most recently S10-19's v39) — every fixture DB this
    // suite opens now migrates further than when the assertion was written; this suite only
    // owns the v32->v33 step itself, not any later one.
    expect(() => {
      orchestrationDb = new OrchestrationDb(dbPath)
    }).not.toThrow()
    const raw = rawInspect(dbPath)

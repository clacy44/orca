...
export const CHAIRS_RESTORE_METHODS: RpcMethod[] = [
  defineMethod({
    ...
    handler: async (params, ctx) => {
      ...
      // [Wave 2 contract A7] Same `chairs-manifest:<host>` lock key chair-succession-accept.ts uses.
      return withPaneLock(`chairs-manifest:${hostId}`, async () => {
  ...
  defineMethod({
    ...
    handler: async (params, ctx) => {
      ...
      const hostId = hostIdFor(runtime)
      const path = params.manifestPath ?? defaultManifestPath()
      // [Wave 2 contract A7] Same lock key as restore/succession-confirm.
      return withPaneLock(`chairs-manifest:${hostId}`, () =>
        exportChairsManifest(params, path, db, hostId)
      )
    }
  })
]

type ExportSkipRow = { name: string; reason: 'no_pane' | 'no_launch_row' | 'no_worktree' }
type PriorSuccessionFields = {
  succession?: ChairsManifestEntry['succession']
  launchArgs?: string[]
}

async function exportChairsManifest(

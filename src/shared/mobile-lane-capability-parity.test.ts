...
import { AGENT_IDENTITY_LANES_RUNTIME_CAPABILITY } from './protocol-version'
...
describe('mobile lane capability literal', () => {
  it('equals the host-advertised runtime capability', () => {
    const source = readFileSync(
      join(__dirname, '..', '..', 'mobile', 'src', 'accounts', 'lane-delegated-switch-request.ts'),
      'utf-8'
    )
    const declared = /AGENT_IDENTITY_LANES_CAPABILITY\s*=\s*'([^']+)'/.exec(source)?.[1]

    expect(declared).toBe(AGENT_IDENTITY_LANES_RUNTIME_CAPABILITY)
  })
})
